      ...
      <div className="max-w-7xl mx-auto">
        <header className="flex flex-col sm:flex-row justify-between items-start sm:items-end mb-8 md:mb-12 lg:mb-16 relative gap-4 sm:gap-0">
          ...
          >
            <div className="flex items-center gap-3 mb-3 md:mb-4">
              <div className="w-8 h-8 md:w-10 md:h-10 bg-primary rounded-xl md:rounded-2xl flex items-center justify-center text-primary-foreground shadow-xl shadow-primary/20">
                <Box size={18} className="md:hidden" />
                <Box size={24} className="hidden md:block" />
              </div>
              <span className="text-[10px] md:text-xs font-black text-primary uppercase tracking-[0.3em] font-display">System Core</span>
            </div>
            <h1 className="text-3xl sm:text-4xl md:text-5xl lg:text-6xl font-[900] text-foreground tracking-tight font-display leading-[0.9]">
              API <span className="text-transparent bg-clip-text bg-gradient-to-r from-primary to-primary/40">Schemas</span>
            </h1>
            <p className="text-muted-foreground mt-3 md:mt-6 text-sm md:text-base lg:text-lg font-bold max-w-md leading-relaxed">
        ...
        {loading ? (
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6 md:gap-10">
            {[1, 2, 3].map((i) => (
              <div key={i} className="h-52 md:h-64 bg-card animate-pulse rounded-[2rem] md:rounded-[2.5rem] border border-border shadow-sm" />
            ))}
          </div>
        ) : (
          <motion.div
          ...
          >
            <AnimatePresence>
              {forms.map((form, idx) => (
                  <motion.div
                    ...
                    whileHover={{ y: -10 }}
                    className="bg-card border border-border rounded-[2rem] md:rounded-[2.5rem] p-6 md:p-10 shadow-[0_10px_50px_rgba(0,0,0,0.03)] hover:shadow-[0_40px_80px_rgba(79,70,229,0.12)] transition-all duration-500 group relative overflow-hidden"
                  >
                    <div className="flex justify-between items-start mb-6 md:mb-10 relative z-10">
                      ...
                      <div className="w-10 h-10 md:w-12 md:h-12 bg-muted rounded-xl md:rounded-2xl flex items-center justify-center text-muted-foreground/50 group-hover:bg-primary group-hover:text-primary-foreground transition-all duration-500 group-hover:rotate-12">
                    ...
                    <div className="relative z-10">
                      <h2 className="text-xl md:text-2xl font-black text-foreground mb-2 truncate font-display group-hover:text-primary transition-colors">
                        {form.name}
                      </h2>
                      <div className="flex items-center gap-2 mb-6 md:mb-10">
                        <Search size={10} className="md:hidden text-muted-foreground/50" />
                        <Search size={12} className="hidden md:block text-muted-foreground/50" />
                        <span className="text-[10px] md:text-[11px] text-muted-foreground font-bold uppercase tracking-widest italic truncate block">
                          {form.slug}
                        </span>
                      </div>
                    </div>
                    ...
                    <div className="grid grid-cols-3 gap-2 md:gap-3 relative z-10 mb-3 md:mb-4">
                      <Button
                        ...
                        className="w-full h-11 md:h-14 text-xs md:text-sm rounded-xl md:rounded-2xl bg-muted/50 hover:bg-card"
                      >
                    ...
                    <Button
                      ...
                      className="w-full h-10 md:h-12 rounded-lg md:rounded-xl text-muted-foreground border-border hover:border-primary/40 hover:text-primary bg-muted/10 mb-3 md:mb-4 text-xs md:text-sm"
                    >
                    ...
                    <div className="grid grid-cols-[1fr_40px] md:grid-cols-[1fr_50px] gap-2 md:gap-3 relative z-10">
                      <Button
                        variant="outline"
                        size="sm"
                        className="w-full h-10 md:h-12 rounded-lg md:rounded-xl text-muted-foreground border-border hover:border-primary/40 hover:text-primary bg-muted/10 text-xs md:text-sm"
                    ...
                    <Layers className="absolute -right-8 -bottom-8 w-32 md:w-40 h-32 md:h-40 text-border/50 group-hover:text-primary/10 transition-colors duration-500 -rotate-12 pointer-events-none" />
                  </motion.div>
              ))}
            </AnimatePresence>
            ...
            {forms.length === 0 && (
              <motion.div
                initial={{ opacity: 0 }}
                animate={{ opacity: 1 }}
                className="col-span-full py-20 md:py-32 bg-card rounded-[3rem] md:rounded-[4rem] border-4 border-dashed border-border flex flex-col items-center justify-center text-center px-6 md:px-12"
              >
                <div className="w-16 h-16 md:w-24 md:h-24 bg-primary/10 text-primary rounded-[1.5rem] md:rounded-[2rem] flex items-center justify-center mb-6 md:mb-8 shadow-xl shadow-primary/20">
                  <Plus size={32} className="md:hidden" />
                  <Plus size={48} className="hidden md:block" />
                </div>
                <h3 className="text-2xl md:text-3xl font-black text-foreground font-display tracking-tight mb-3 md:mb-4">No Schema Entities</h3>
                <p className="text-muted-foreground max-w-sm mb-8 md:mb-10 font-bold uppercase text-[9px] md:text-[10px] tracking-[0.2em] leading-relaxed">
